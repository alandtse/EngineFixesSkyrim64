...
#define EF_SETTINGS_FIXES(X)                                                                                                                                                                                                                                      \
    ...
    X(Bool, bBatchRendererAlphaGeometryGroupOverflow, true, "guards StartGroupingAlphas' unbounded alpha-group slot allocator against writing past the array once iBatchRendererAlphaGeometryGroupLimit groups are live")                                         \
    X(U32, iBatchRendererAlphaGeometryGroupLimit, 0, "max live alpha-sorted render groups per frame before new ones are dropped; 0 = array capacity (512, VR 1024) minus a margin, larger values are clamped")                                                    \
    X(Bool, bCullingProcessAppendVirtualPoolGuard, true, "guards BSCullingProcess::AppendVirtual against a null write when its shadow-caster free pool is exhausted; the append is dropped")                                                                      \
    ...
    X(Bool, bCombatGroupStalePointerCrash, true, "guards Actor::GetCombatGroup against handing back a stale/implausible pointer when read off the main thread during a combat-group reassignment race")                                                           \
    ...
    X(Bool, bShadowLightCrossThreadFreeCrash, true, "fixes a crash when a shadow light released on a job thread, e.g. by the console coc command, is freed while the render thread still uses it; frees are deferred to the end of the frame")                    \
