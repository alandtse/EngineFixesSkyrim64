...
#define EF_SETTINGS_FIXES(X)                                                                                                                                                                                                                                             \
    ...
    X(Bool, bGCArrayCleanupBug, true, "(SE/AE-only for now) fixes the Papyrus VM's incremental garbage collector sometimes exiting its array/object cleanup loop after a single entry, forgoing the rest of its time budget and causing progressive script lag/hitches") \
