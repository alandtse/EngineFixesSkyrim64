...
namespace Fixes::MemoryAccessErrors
{
    namespace detail
    {
        ...
        inline bool Load(RE::BGSShaderParticleGeometryData* a_this, RE::TESFile* a_file)
        {
            ...
            if (hasData) {
                ...
                if (particleDensity.f > 10.0f) {
                    if (REL::Module::IsVR())
                        a_this->GetVRRuntimeData().data[static_cast<std::uint32_t>(DataID::kParticleDensity)].value.f = 10.0f;
                    else
                        a_this->GetRuntimeData().data[static_cast<std::uint32_t>(DataID::kParticleDensity)].f = 10.0f;
                }
            }
        ...
        inline void InstallBSShadowDirectionalLightUseAfterFree()
        {
            ...
            // AE1799 recompile shifted this instruction from +0x1BED to +0x1BFD (same
            // `MOV R9,[RBP+...]`, no other layout change).
